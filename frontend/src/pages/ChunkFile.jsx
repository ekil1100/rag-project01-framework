...
const ChunkFile = () => {
    ...
    const fetchLoadedDocuments = async () => {
        try {
            ...
            const chunkedResponse = await fetch(
                `${apiBaseUrl}/documents?type=chunked`,
            )
            ...
            if (
                !chunkedData.documents ||
                !Array.isArray(chunkedData.documents)
            ) {
                ...
                chunkedData.documents.map(async (doc) => {
                    try {
                        ...
                        if (!detailResponse.ok) {
                            console.error(
                                `Error fetching details for ${doc.name}:`,
                        ...
                        console.error(
                            `Error processing document ${doc.name}:`,
                            error,
                        )
                        return doc
                    }
    ...
    const handleChunk = async () => {
        ...
        try {
            const docId = selectedDoc.endsWith('.json')
                ? selectedDoc
                : `${selectedDoc}.json`
    ...
    const handleDeleteDocument = async (docName) => {
        try {
            const response = await fetch(
                `${apiBaseUrl}/documents/${docName}?type=chunked`,
                {
                    method: 'DELETE',
                },
            )
                        ...
                        <div className='w-full'>
                            <div className='mb-4 p-3 border rounded bg-gray-100'>
                                ...
                                <div className='text-sm text-gray-600'>
                                    ...
                                    <p>
                                        Loading Method: {chunks.loading_method}
                                    </p>
                                    <p>
                                        Chunking Method:{' '}
                    ...
                    <div className='p-4 border rounded-lg bg-white shadow-sm'>
                        ...
                        <div className='mb-4'>
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    setChunkingOption(e.target.value)
                                }
                                className='block w-full p-2 border rounded'
                            >
                                <option value='by_pages'>By Pages</option>
                                <option value='fixed_size'>Fixed Size</option>
                                <option value='by_paragraphs'>
                                    By Paragraphs
                                </option>
                                <option value='by_sentences'>
                                    By Sentences
                                </option>
                            </select>
                        </div>
