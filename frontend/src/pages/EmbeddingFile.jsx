    ...
    const handleEmbed = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    errorData.detail || 'Failed to create embeddings',
                )
            }
            ...
            setStatus(
                `Embedding completed successfully! Saved to: ${data.filepath}`,
            )
    ...
    const handleDeleteEmbedding = async (docName) => {
        try {
            const response = await fetch(
                `${apiBaseUrl}/embedded-docs/${docName}`,
                {
                    method: 'DELETE',
                },
            )
                                ...
                                {embeddings.map((embedding, idx) => (
                                    ...
                                    >
                                        <div className='font-medium text-sm text-gray-500 mb-1'>
                                            Chunk {embedding.metadata.chunk_id}{' '}
                                            of {embedding.metadata.total_chunks}
                                        </div>
                                        <div className='text-xs text-gray-400 mb-2'>
                                            Document:{' '}
                                            {embedding.metadata.filename ||
                                                embedding.metadata
                                                    .document_name ||
                                                'N/A'}{' '}
                                            | Page:{' '}
                                            {embedding.metadata.page_number ||
                                                'N/A'}{' '}
                                            | Page Range:{' '}
                                            {embedding.metadata.page_range ||
                                                'N/A'}
                                        </div>
                                        <div className='text-xs text-gray-400 mb-2'>
                                            Model:{' '}
                                            ...
                                            | Timestamp:{' '}
                                            {new Date(
                                                embedding.metadata.embedding_timestamp,
                                            ).toLocaleString()}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div className='text-sm text-gray-600 mt-1'>
                                                ...
                                                <p>
                                                    Created:{' '}
                                                    {new Date(
                                                        doc.metadata?.embedding_timestamp,
                                                    ).toLocaleString()}
                                                </p>
                                            </div>
                                        </div>
                    ...
                    <div className='p-4 border rounded-lg bg-white shadow-sm'>
                        ...
                        <div className='mt-4'>
                            ...
                            <select
                                ...
                                className='block w-full p-2 border rounded'
                            >
                                {modelOptions[embeddingProvider].map(
                                    (model) => (
                                        <option
                                            key={model.value}
                                            value={model.value}
